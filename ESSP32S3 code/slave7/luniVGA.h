...
// #include "spiram.h" // Commented out as in original
...
const VGAMode *mode;
const int bufferCount = 2; // Changed to 2 for double buffering
...
void vgaDot(uint32_t x, uint32_t y, uint8_t r, uint8_t g, uint8_t b)
{
	...
	if(bitCount == 8)
		dmaBuffer->getLineAddr8(y, backBuffer)[x] = (r >> 5) | ((g >> 5) << 3) | ((b >> 6) << 6);
	else if(bitCount == 16)
...
// // Function to clear the current framebuffer
// void clearVGA() // Parameters are ignored
// {
//     uint16_t colorValue = 0; // Always clear to 0 (black)

//     int totalLinesInCurrentBuffer = dmaBuffer->lines; 
//     size_t bytesPerLine = dmaBuffer->lineSize; 

    
//         for (int y = 0; y < totalLinesInCurrentBuffer; ++y) {
//             uint16_t* linePtr = dmaBuffer->getLineAddr16(y, backBuffer); 
//             for (int x = 0; x < mode->hRes; ++x) { 
//                 linePtr[x] = colorValue;
//             }
//         }
